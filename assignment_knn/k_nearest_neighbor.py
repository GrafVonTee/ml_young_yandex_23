...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        X_test_squared = np.sum(np.array(X**2), axis=1)
        X_train_squared = np.sum(np.array(self.X_train**2), axis=1)
        cross_term = np.dot(X, self.X_train.T)
        dists = np.sqrt(X_test_squared[:, np.newaxis] - 2 * cross_term + X_train_squared)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            most_common_label = np.min(candidates)
